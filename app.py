...
def update_team_settings():
    ...
    if username == 'admin':
        # Admin update: expects old_team_name and new team_name
        ...
        try:
            ...
            db.commit()
            flash(f"Team name updated from {old_team_name} to {new_team_name}.")
        except Exception:
...
def submit_answer():
    ...
    if team_name:
        ...
        if result:
            ...
            logging.info(f"Team {team_name} solved puzzle {box_number}.")
        else:
            logging.info(f"Team {team_name} attempted puzzle {box_number} but answer was incorrect.")
    else:
...
def sync_submissions():
    ...
    for submission in data:
        ...
        if team_name:
            ...
            if result:
                cursor.execute('UPDATE teams SET score = score + 1 WHERE team_name = ?', (team_name,))
                logging.info(f"Team {team_name} solved puzzle {box_number} via sync.")
            else:
                logging.info(f"Team {team_name} attempted puzzle {box_number} via sync but answer was incorrect.")
        else:
            cursor.execute('UPDATE puzzles SET solved = 1 WHERE box_number = ?', (box_number,))
        db.commit()
...
def delete_puzzle():
    ...
    db.commit()
    logging.info(f"Puzzle {box_number} deleted by admin.")
    return jsonify({'message': f'Puzzle {box_number} deleted.'})
...
def edit_puzzle():
    ...
    db.commit()
    logging.info(f"Puzzle {box_number} edited by admin.")
...
# --- NEW: Define admin dashboard route with endpoint 'admin' ---
